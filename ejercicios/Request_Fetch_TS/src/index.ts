...
import { Character, ApiListResponse } from './models/models';

const BASE_URL = process.env.BASE_URL;

// Servicio de personajes
const characterService = new ApiService<ApiListResponse<Character>>(
  `${BASE_URL}/character`
);
...
async function main() {
  ...
  // Acceder a results
  if (characters.data) {
    console.log('Lista real:', characters.data.results);
  }

  // Obtener uno
  const character = await characterService.getOne(1);

  console.log('Personaje 1:');
  console.log(character);

  // ID inválido
  const invalid = await characterService.getOne(99999);

  console.log('Personaje inválido:');
  console.log(invalid);
}
